"""Bot 3 — "Il Disciplinato": regole semplici, cuore nel risk management.

Entrata su breakout (chiusura sopra il massimo delle ultime N barre).
Il segnale d'ingresso e' deliberatamente semplice: qui il punto non e'
"indovinare" il movimento, ma dimensionare la posizione in base al
rischio e uscire con disciplina via stop-loss / take-profit.
"""

from __future__ import annotations

from .base import Action, Bot, MarketContext, Signal


class DisciplinedBot(Bot):
    """Position sizing basato sul rischio per trade, stop-loss, take-profit, esposizione massima."""

    def __init__(
        self,
        breakout_period: int = 20,
        risk_per_trade_pct: float = 0.01,
        stop_loss_pct: float = 0.08,
        max_exposure_pct: float = 0.3,
        trend_filter_period: int = 50,
        name: str = "Il Disciplinato",
    ):
        if breakout_period < 1:
            raise ValueError("breakout_period deve essere >= 1")
        if trend_filter_period < 0:
            raise ValueError("trend_filter_period deve essere >= 0 (0 = filtro disattivato)")
        for label, value in (
            ("risk_per_trade_pct", risk_per_trade_pct),
            ("stop_loss_pct", stop_loss_pct),
            ("max_exposure_pct", max_exposure_pct),
        ):
            if not (0 < value <= 1):
                raise ValueError(f"{label} deve essere in (0, 1]")
        self.breakout_period = breakout_period
        self.risk_per_trade_pct = risk_per_trade_pct
        # `stop_loss_pct` e' la distanza del trailing stop dal massimo raggiunto: protegge
        # dalle perdite e, salendo col prezzo, lascia correre i vincitori invece di tagliarli
        # a un target fisso (l'errore classico "taglia i guadagni, tieni le perdite").
        self.stop_loss_pct = stop_loss_pct
        self.max_exposure_pct = max_exposure_pct
        # Filtro di trend: si entra solo se il prezzo e' sopra la sua media a N barre
        # (mercato in salita). Comprare breakout in un downtrend significa "prendere
        # coltelli che cadono": questo filtro evita le entrate contro-tendenza.
        self.trend_filter_period = trend_filter_period
        self.name = name
        # Massimo prezzo raggiunto da quando la posizione e' aperta, per symbol (trailing stop).
        self._peak_price: dict[str, float] = {}

    def on_data(self, context: MarketContext) -> Signal:
        history = context.history
        min_bars = self.breakout_period + 1

        if len(history) < min_bars:
            return Signal(
                Action.HOLD,
                0,
                f"HOLD: dati insufficienti ({len(history)}/{min_bars} barre necessarie per il breakout)",
            )

        if context.position_qty > 0:
            return self._manage_open_position(context)
        return self._evaluate_entry(context)

    def _manage_open_position(self, context: MarketContext) -> Signal:
        symbol = context.symbol
        avg_price = context.position_avg_price
        last_bar = context.history.iloc[-1]

        # Lo stop di questa barra si basa sul massimo raggiunto FINO ALLA BARRA PRECEDENTE
        # (parte dal prezzo di carico): cosi' non c'e' contraddizione intra-barra (un nuovo
        # massimo e uno stop dallo stesso massimo nella medesima candela). Il picco si
        # aggiorna dopo, per la barra successiva.
        prev_peak = self._peak_price.get(symbol, avg_price)
        trailing_stop = prev_peak * (1 - self.stop_loss_pct)

        if last_bar["low"] <= trailing_stop:
            self._peak_price.pop(symbol, None)  # posizione chiusa: dimentica il picco
            gain_from_entry = (trailing_stop / avg_price - 1) * 100
            return Signal(
                Action.SELL,
                context.position_qty,
                f"SELL {context.position_qty:.6f} {context.symbol}: trailing stop colpito "
                f"(minimo {last_bar['low']:.2f} <= stop {trailing_stop:.2f}, "
                f"-{self.stop_loss_pct * 100:.1f}% dal massimo {prev_peak:.2f}; entry {avg_price:.2f}, "
                f"{gain_from_entry:+.1f}% sull'ingresso)",
            )

        self._peak_price[symbol] = max(prev_peak, float(last_bar["high"]))
        return Signal(
            Action.HOLD,
            0,
            f"HOLD: posizione aperta su {context.symbol}, trailing stop a {trailing_stop:.2f} "
            f"({self.stop_loss_pct * 100:.1f}% sotto il massimo {prev_peak:.2f})",
        )

    def _evaluate_entry(self, context: MarketContext) -> Signal:
        history = context.history
        entry_price = context.last_price

        # Filtro di trend: niente entrate se il prezzo e' sotto la media di lungo periodo.
        if self.trend_filter_period > 0 and len(history) >= self.trend_filter_period:
            trend_sma = history["close"].tail(self.trend_filter_period).mean()
            if entry_price < trend_sma:
                return Signal(
                    Action.HOLD,
                    0,
                    f"HOLD: trend ribassista (prezzo {entry_price:.2f} < SMA{self.trend_filter_period} "
                    f"{trend_sma:.2f}), niente entrate contro-tendenza",
                )

        prior_highs = history["high"].iloc[-(self.breakout_period + 1) : -1]
        breakout_level = prior_highs.max()

        if entry_price <= breakout_level:
            return Signal(
                Action.HOLD,
                0,
                f"HOLD: nessun breakout (chiusura {entry_price:.2f} <= massimo "
                f"{self.breakout_period} barre {breakout_level:.2f})",
            )

        stop_distance = entry_price * self.stop_loss_pct
        risk_amount = context.cash * self.risk_per_trade_pct
        risk_based_qty = risk_amount / stop_distance
        exposure_cap_qty = (context.cash * self.max_exposure_pct) / entry_price
        affordable_qty = context.cash / entry_price

        quantity = min(risk_based_qty, exposure_cap_qty, affordable_qty)

        if quantity <= 0:
            return Signal(Action.HOLD, 0, "HOLD: breakout confermato ma cassa insufficiente per entrare")

        capped_by = "esposizione massima" if exposure_cap_qty < risk_based_qty else "rischio per trade"
        return Signal(
            Action.BUY,
            quantity,
            f"BUY {quantity:.6f} {context.symbol}: breakout sopra massimo {self.breakout_period} barre "
            f"({breakout_level:.2f}), size da {capped_by} ({self.risk_per_trade_pct * 100:.1f}% rischio, "
            f"trailing stop -{self.stop_loss_pct * 100:.1f}%, max esposizione {self.max_exposure_pct * 100:.0f}%)",
        )
